...
test('o visual final usa um tabuleiro monobloco construído em CSS', () => {
  ...
  assert.match(css, /V0\.0\.29 — tabuleiro menos alto, filas mais próximas e madeira mais polida/);
  assert.match(css, /aspect-ratio:\s*1200 \/ 540/);
  assert.match(css, /inset 18px 16px 22px/);
});
...
test('a distância entre as duas filas de buracos foi reduzida', () => {
  assert.match(css, /north-row \.pit:nth-child\(1\) \{ left: 76\.15%; top: 15\.4%; \}/);
  assert.match(css, /south-row \.pit:nth-child\(1\) \{ left: 76\.15%; top: 57\.0%; \}/);
});
