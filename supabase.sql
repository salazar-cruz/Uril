...
alter table public.uril_rooms add column if not exists room_kind text not null default 'official';
alter table public.uril_rooms add column if not exists private_room boolean not null default false;
alter table public.uril_rooms add column if not exists invited_user_id uuid references auth.users(id) on delete set null;
alter table public.uril_rooms add column if not exists invite_token uuid not null default gen_random_uuid();
